...
class Controller(object):
    ...
    def __init__(self, wheel_base, steer_ratio, min_speed, max_lat_accel, max_steer_angle, brake_deadband,
        accel_limit, decel_limit, vehicle_mass, wheel_radius, fuel_capacity, sample_rate):
        ...
        self.lowpass_filter      = LowPassFilter(0.15, self.sample_rate)
    ...
    def control(self, target_linear_velocity, target_angular_velocity, current_velocity):
        ...
        raw_accel = self.accel_controller.step(vel_error, self.sample_rate)
        ...
        if accel > 0.0:
            throttle = self.throttle_controller.step(accel, self.sample_rate)
